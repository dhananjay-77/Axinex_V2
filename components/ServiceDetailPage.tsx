...
export default function ServiceDetailPage({ service }: { service: ServiceItem }) {
  const theme = toneClasses[service.tone]
    ...
    <main className="overflow-hidden pt-16 md:pt-20">
      <section className="relative mx-auto grid min-h-[calc(100svh-5rem)] max-w-container-max items-center gap-10 px-margin-mobile py-16 md:grid-cols-12 md:px-margin-desktop md:py-20">
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          ...
          <div className={`absolute right-8 top-20 h-72 w-72 rounded-full blur-[120px] ${service.tone === 'blue' ? 'bg-primary/20' : 'bg-vibrant-red/20'}`} />
        ...
        <div className="relative z-10 md:col-span-7">
          <div className="mb-stack-md inline-flex items-center gap-3 rounded-full border border-outline-variant/40 bg-white/80 px-4 py-2 shadow-sm backdrop-blur">
            <span className={`flex h-9 w-9 items-center justify-center rounded-full ${theme.accentSoft} ${theme.text}`}>
              <InlineIcon name={service.icon} className="h-5 w-5" />
            </span>
          ...
          <div className="mt-stack-lg grid max-w-2xl gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-outline-variant/35 bg-white/85 p-4 shadow-sm backdrop-blur">
              <div className={`font-headline-md text-headline-md ${theme.text}`}>{service.heroStatOne}</div>
              <div className="mt-1 font-label-md text-xs uppercase tracking-[0.18em] text-on-surface-variant">{service.heroStatOneLabel}</div>
            </div>
            <div className="rounded-2xl border border-outline-variant/35 bg-white/85 p-4 shadow-sm backdrop-blur">
              <div className="font-headline-md text-headline-md text-primary">{service.heroStatTwo}</div>
              <div className="mt-1 font-label-md text-xs uppercase tracking-[0.18em] text-on-surface-variant">{service.heroStatTwoLabel}</div>
            </div>
          </div>

          <div className="mt-stack-md flex flex-wrap gap-3">
            {service.quickPoints.map((point) => (
              <span
                key={point}
          ...
          <div className="mt-stack-lg flex flex-col gap-4 sm:flex-row sm:flex-wrap">
            <Link
              href="/services"
              className="inline-flex min-h-[48px] items-center justify-center rounded-lg border border-primary px-8 font-label-md text-label-md text-primary transition-all hover:-translate-y-[1px] hover:bg-primary/5 hover:shadow-md"
            >
              Back to Services
            </Link>
            <Link
              href="/contact"
              className="inline-flex min-h-[48px] items-center justify-center rounded-lg bg-vibrant-red px-8 font-label-md text-label-md text-on-primary transition-all hover:-translate-y-[1px] hover:opacity-90 hover:shadow-md"
            >
                ...
                <div className="absolute bottom-5 left-5 right-5 rounded-2xl border border-white/20 bg-white/90 p-4 shadow-lg backdrop-blur">
                  <div className="flex items-center gap-3">
                    <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${theme.accent} text-white`}>
                      <InlineIcon name="rocket" className="h-5 w-5" />
                    </span>
                    <div>
                      <div className="font-label-md text-label-md text-primary">Delivery Blueprint</div>
                      <p className="mt-1 text-sm leading-5 text-on-surface-variant">{service.process[0].title}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-4 p-5">
                <div className="rounded-2xl border border-outline-variant/30 bg-surface-container-lowest p-4">
                  <div className="mb-3 flex items-center gap-2 font-label-md text-label-md text-primary">
                    <InlineIcon name={service.icon} className="h-5 w-5 text-vibrant-red" />
                    Capability Path
                  </div>
        ...
        <div className="mx-auto max-w-container-max px-margin-mobile md:px-margin-desktop">
          <div className="max-w-3xl">
            ...
            <h2 className="font-headline-lg text-headline-lg text-primary">
              A focused service model for dependable delivery.
            </h2>
      ...
      <section className="py-section-gap">
        <div className="mx-auto max-w-container-max px-margin-mobile md:px-margin-desktop">
          <div className="max-w-3xl">
            ...
            <h2 className="font-headline-lg text-headline-lg text-primary">
              The delivery friction this service is built to remove.
            </h2>
          </div>
      ...
      <section className="bg-surface-container-lowest py-section-gap">
        <div className="mx-auto max-w-container-max px-margin-mobile md:px-margin-desktop">
          <div className="max-w-3xl">
            ...
            <h2 className="font-headline-lg text-headline-lg text-primary">
              The core technical capabilities included in this service.
            </h2>
          </div>
            ...
            {service.capabilities.map((item) => (
              <article key={item.title} className="rounded-2xl border border-outline-variant/25 bg-white p-6 shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg">
                <div className={`mb-4 inline-flex h-12 w-12 items-center justify-center rounded-xl ${theme.accentSoft} ${theme.text}`}>
                  <InlineIcon name={item.icon ?? 'check_circle'} className="h-6 w-6" />
                </div>
      ...
      <section className="py-section-gap">
        <div className="mx-auto max-w-container-max px-margin-mobile md:px-margin-desktop">
          <div className="max-w-3xl">
            ...
            <h2 className="font-headline-lg text-headline-lg text-primary">
              Practical outcomes your team can expect from the engagement.
            </h2>
          </div>
            ...
            {service.benefits.map((item) => (
              ...
              >
                <div className={`mb-4 inline-flex h-11 w-11 items-center justify-center rounded-full ${theme.accent} text-white`}>
                  <InlineIcon name={item.icon ?? 'check_circle'} className="h-5 w-5" />
                </div>
                <h3 className="font-label-md text-lg text-primary">
                  {item.title}
                </h3>
      ...
      <section className="bg-[#eef5fb] py-section-gap">
        <div className="mx-auto max-w-container-max px-margin-mobile md:px-margin-desktop">
          <div className="max-w-3xl">
            ...
            <h2 className="font-headline-lg text-headline-lg text-primary">
              A structured delivery path from discovery to improvement.
            </h2>
          </div>
      ...
      <section className="py-section-gap text-center">
        <div className="mx-auto max-w-[1000px] justify-center px-margin-mobile md:px-margin-desktop">
          <div className="rounded-3xl border border-outline-variant/30 bg-surface-container-low p-stack-lg md:p-16">
            ...
            <div className="mt-stack-lg flex flex-col justify-center gap-4 sm:flex-row">
              ...
              <Link href="/services" className="inline-flex min-h-[52px] items-center justify-center rounded-lg border border-primary px-8 font-label-md text-label-md text-primary transition-all hover:bg-primary/5">
